...
namespace CoreAPI.Repository.BASE
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        private ISqlSugarClient _db
        {
            get
            {
                 ...
                 */
                if (Common.Appseting.Appsettings.app(new string[] { "MutiDBEnabled" }).ObjToBool())
                {
        ...
        /// <param name="strOrderByFileds">排序字段，如name asc,age desc</param>
        /// <returns></returns>
        public async Task<PageModels<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression, int intPageIndex = 1, int intPageSize = 20, string strOrderByFileds = null)
        {
            ...
            return new PageModels<TEntity>() { dataCount = totalCount, pageCount = pageCount, page = intPageIndex, PageSize = intPageSize, data = list };
        }
        ...
        /// <returns></returns>
        public async Task<PageModels<TResult>> QueryTabsPage<T, T2, TResult>(
            ...
            string strOrderByFileds = null)
        {
            ...
            var list = await _db.Queryable<T, T2>(joinExpression)
             ...
             .ToPageListAsync(intPageIndex, intPageSize, totalCount);
            int pageCount = Math.Ceiling(totalCount.ObjToDecimal() / intPageSize.ObjToDecimal()).ObjToInt();
            return new PageModels<TResult>() { dataCount = totalCount, pageCount = pageCount, page = intPageIndex, PageSize = intPageSize, data = list };
        }
