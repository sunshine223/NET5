...
namespace CoreAPI.Extensions.Middlewares
{
    ...
    public static class AllServicesMildd
    {
        public static void UseAllServicesMildd(this IApplicationBuilder app, IServiceCollection _services)
        {
            ...
            tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "CoreAPI.Service.dll")).GetTypes().ToList());
            tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "CoreAPI.Repository.dll")).GetTypes().ToList());
